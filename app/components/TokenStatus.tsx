...
import {
  TokenStatus as TokenStatusData,
  fetchTokenStatus,
} from '../lib/api'
...
export default function TokenStatus() {
  ...
  useEffect(() => {
    ...
    // Poll every 3 seconds instead of every 1 second.
  ...
  return (
    <div className="card tokenCard">
      ...
      {tokens.length ? (
                      ...
                      {token.invalid
                        ...
                        : token.cooldown_active
                          ? `Cooldown ${formatDuration(
                              token.cooldown_seconds
                            )}`
                          : token.initialized
                            ? `Reset ${formatDuration(
                                token.reset_seconds
                              )}`
                            : 'Waiting for response'}
                    </td>
        ...
        <div className="empty tokenEmpty">
          No GitHub tokens configured.
        </div>
      )}

      <p className="tokenFootnote">
        Usage and reset times come from
        GitHub's live rate-limit response for
        each token. Cooldown appears only
        after GitHub reports throttling for
        that token.
      </p>
    </div>
  )
