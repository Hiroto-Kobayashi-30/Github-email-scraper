...
export async function startRun(form: RunForm): Promise<{ accepted: boolean }> {
...
export async function cancelRun(): Promise<{ cancelled: boolean }> {
  const r = await fetch(`${API}/api/runs/cancel`, { method: 'POST' })
  if (!r.ok) throw new Error('Failed to cancel run')
  return r.json()
}

export async function fetchHistory(limit = 100, offset = 0, q = ''): Promise<{ total: number; records: HistoryRecord[] }> {
